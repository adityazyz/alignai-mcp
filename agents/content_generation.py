...
def smart_match_participant_id(name: str, participants: list) -> str:
    ...
    Logs warnings for ambiguous or unmatched names.
    """
    ...
    for participant in participants:
        ...
        if 'firstName' in participant and 'lastName' in participant:
            ...
            if name_lower == first_name or name_lower == last_name:
                participant_id = participant.get("id", participant.get("email", ""))
                partial_matches.append(participant_id)
    ...
    if len(partial_matches) == 1:
        logger.debug(f"Single partial match: '{name}' -> ID: '{partial_matches[0]}'")
        return partial_matches[0]
    elif len(partial_matches) > 1:
        logger.warning(f"Multiple partial matches for name '{name}': {partial_matches}. Using first match.")
        return partial_matches[0]
    
    logger.warning(f"No match found for name '{name}'. Using default ID.")
    return get_default_participant_id(participants)

def smart_match_participant_emails(names: List[str], participants: list) -> str:
    """
    ...
    if not names or not participants:
        ...
        return get_default_participant_email(participants)
        ...
        for participant in participants:
            ...
            if 'firstName' in participant and 'lastName' in participant:
                ...
                if name_lower == full_name:
                    ...
                    if email and email not in matched_emails and email != "ai":
...
def get_default_participant_email(participants: list) -> str:
    """Get a default valid participant email for fallback cases."""
    if not participants:
...
async def generate_content(transcription: str, content_details: dict, attendees: list, participants: list, organization_id: str, department_id: str) -> List[GeneratedContent]:
    """Core content generation logic with enhanced user ID matching for multiple users."""
    try:
        # Validate that participants list contains unique IDs
        participant_ids = [p.get("id", p.get("email", "")) for p in participants if isinstance(p, dict)]
        unique_ids = set([pid for pid in participant_ids if pid and pid != "ai"])
        if len(unique_ids) < len([p for p in participant_ids if p and p != "ai"]):
            logger.warning("Duplicate participant IDs detected. Ensuring unique ID assignment.")

        # Enhanced prompt to emphasize accurate user identification
        prompt = ChatPromptTemplate.from_messages([
            ("system", """You are an expert content generator for business meetings. Based on the meeting transcription, generate appropriate follow-up content such as emails, documents, or reports.

For each content item you generate:
1. Maintain a professional tone
2. Include relevant details from the meeting
3. Identify who should create/send this content and who should receive it based on the meeting context
4. Structure the content clearly and actionably
5. Ensure creator and recipient names are matched to specific participant IDs or emails from the provided participants list
6. If multiple users are mentioned in the transcription, generate separate content items for each user when appropriate
...
- content: string (the main content body)
- subject: string (subject line for emails, title for documents)
- createdForName: string (name of the person who should create/send this content - must match a participant name, use "default" if unclear)
- recipientNames: array of strings (names of all recipients - must match participant names, use ["default"] if unclear)

Pay special attention to identifying multiple users when mentioned in the transcription. Avoid using a single team ID for multiple users. If multiple people are responsible for or should receive content, include their specific names in createdForName or recipientNames.

If only one content item is needed, return it as an array with one object. If no content is needed, return an empty array []."""),
...
Generate appropriate follow-up content for this meeting. Ensure creator and recipient names match specific participants. Return as JSON array (can be empty if no content needed).""")
        ])
        ...
        # Convert to GeneratedContent objects with enhanced ID matching
        generated_content_list = []
        default_creator = get_default_participant_id(participants)
        default_recipient_email = get_default_participant_email(participants)
        ...
        for content_data in content_data_list:
            try:
                ...
                # Match multiple recipients to their email addresses
                recipient_emails = smart_match_participant_emails(recipient_names, participants)
                
                # Prevent using team ID when multiple users are detected
                if len(unique_ids) > 1 and created_for_id == default_creator and created_for_name == "default":
                    logger.warning(f"Multiple users detected but default creator ID used: '{default_creator}'. Transcription may need clearer user assignment.")
                if len(unique_ids) > 1 and recipient_emails == default_recipient_email and all(r == "default" for r in recipient_names):
                    logger.warning(f"Multiple users detected but default recipient email used: '{default_recipient_email}'. Transcription may need clearer recipient assignment.")
...
5. Appropriate structure and formatting
6. Correct identification of creator and recipients

Provide specific suggestions for improvement, especially regarding accurate user identification."""),
...
6. Uses correct participant IDs/emails for creator and recipients

Return the improved content as a JSON object with the same structure."""),
...
Please provide the refined content based on this feedback, ensuring accurate user identification.""")
                    ])
                    ...
                    try:
                        ...
                        if json_match:
                            ...
                            primary_content.content = refined_data.get("content", primary_content.content)
